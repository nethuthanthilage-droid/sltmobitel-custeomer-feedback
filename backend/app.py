...
GMAIL_ADDRESS = os.environ.get("mobitelslt13@gmail.com")
GMAIL_APP_PASSWORD = os.environ.get("SLT$Mobitel123")
...
def save_feedback(feedback: Feedback):
    ...
    try:
        ...
        saved_data = response.data or []

        if not saved_data:

            raise HTTPException(
                status_code=500,
                detail="Feedback was not returned by Supabase after insert."
            )
...
def improve_feedback(
    ...
    try:
        ...
        suggestion = generate_improvement(
            data.model_dump()
        )
...
def negative_feedback_summary(
    ...
    try:
        ...
        summary = generate_negative_summary(
            feedback
        )
...
def ai_summary(
    ...
    try:
        ...
        if total == 0:
            ...
            overall_summary = (
                "There is currently no customer "
                "feedback available for analysis."
            )
        ...
        if office_issue:
            ...
            key_findings.append(
                "Office-environment rating requiring attention: "
                f"{office_issue[0]} "
                f"({office_issue[1]})."
            )
        ...
        if (
            waiting.get("Poor", 0) > 0
            or waiting.get("Very Poor", 0) > 0
        ):
...
def download_customer_feedback(
    ...
    try:
        ...
        document = (
            create_customer_feedback_document(
                feedback
            )
        )
...
def send_email_with_attachment(
    ...
    if not GMAIL_ADDRESS:
        ...
        raise RuntimeError(
            "GMAIL_ADDRESS is not configured in .env"
        )
...
@app.post("/email-customer-feedback")
def email_customer_feedback(
    ...
    try:
        ...
        if not feedback:
            ...
            raise HTTPException(
                status_code=404,
                detail="No customer feedback found."
            )

        # Create fresh document from current database
        document = (
            create_customer_feedback_document(
                feedback
            )
        ...
        send_email_with_attachment(
            ...
            body=(
                ...
                "SLTMobitel customer feedback report.\n\n"

                f"Total feedback records: "
                f"{len(feedback)}\n\n"

                "The report was generated automatically "
                "from the Supabase customer_feedback table.\n\n"
